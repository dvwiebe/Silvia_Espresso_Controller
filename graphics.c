...
bounding_box_t draw_text(char *string, unsigned char x, unsigned char y, unsigned char *font, unsigned char spacing, unsigned char colour) {
	bounding_box_t ret;
	bounding_box_t tmp;
	...
	while (*string != 0) {
		tmp = draw_char(*string++, x, y, font, colour);
...
bounding_box_t draw_char(unsigned char c, unsigned char x, unsigned char y, unsigned char *font, unsigned char colour) {
	unsigned short pos;
	unsigned char width;
	bounding_box_t ret;
...
//Use to draw solid rectangle; can be used to clear sections of screen by setting colour to 0
void draw_filled_rectangle(int x1, int y1, int x2, int y2, char colour )
{
    int y;
    int x;
    for(y = y1; y <= y2 ; y++)
